...
async function getPaginateData(page = 1) {
    await axios.get(`/api/v1/invoices/${selectedCompany.value.id}/trash?page=${page}&sort=${filters.value.sort}&order=${filters.value.orderBy}&search=${filters.value.search}`).then((res) => {
...
function forceDeleteItem(event, id) {
    confirm.require({
        ...
        header: 'Confirmation',
        accept: () => {
            invoiceStore.forceDeleteInvoice(id).then(() => {
...
function restoreItem(id) {
    invoiceStore.restoreInvoice(id).then(() => {
        toast.add({ severity: 'success', summary: 'Successful', detail: 'Invoice restored successfully!', life: 3000 });
        setTimeout(() => {
            window.location.reload();
        }, 2000);
    });
