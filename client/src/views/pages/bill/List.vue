<script setup>
import Create from '@/views/pages/bill/CreateEditModal.vue';
...
async function getPaginateData(page = 1) {
    await axios.get(`/api/v1/bills/${selectedCompany.value.id}?page=${page}&sort=${filters.value.sort}&order=${filters.value.orderBy}&search=${filters.value.search}`).then((res) => {
...
function toggleEditModal(bill) {
    loading.value = true;
    billStore.getBill(bill).then(() => {
        editedItem.value = billStore.bill.data;
        isEdit.value = true;
        showModal.value = !showModal.value;
        loading.value = false;
    });
...
function deleteItem(event, id) {
    confirm.require({
        ...
        header: 'Confirmation',
        accept: () => {
            billStore.deleteBill(id).then(() => {
                toast.add({ severity: 'success', summary: 'Successful', detail: 'Bill deleted successfully!', life: 3000 });
                setTimeout(() => {
...
<template>
    <Toolbar class="col-12 my-3">
        <template #start>
            <p class="text-xl font-bold">Bills List</p>
        </template>
    ...
    <div class="card">
        <DataTable :loading="loading" ref="dt" dataKey="id" :value="paginateList.data" stripedRows removableSort responsiveLayout="scroll" class="p-datatable-sm">
            ...
            <Column field="status">
                ...
                <template #body="slotProps">
                    <Badge v-if="slotProps.data.status === 'draft'" severity="warning" value="Draft" />
                    <Badge v-else-if="slotProps.data.status === 'paid'" severity="success" value="Paid" />
                    <Badge v-else severity="danger" value="Canceled" />
                </template>
            </Column>
            <Column field="discount">
                <template #header>
                    <SortIcon @click="sortItem('discount')" name="Discount" column="discount" :sort="filters.sort" :active-column="filters.orderBy" />
                </template>
            </Column>
            <Column field="withholding.rate">
                <template #header>
                    <SortIcon @click="sortItem('withholding_id')" name="Withholding" column="withholding_id" :sort="filters.sort" :active-column="filters.orderBy" />
                </template>
                <template #body="slotProps">
                    <span v-if="slotProps.data.withholding.rate !== 0">{{ slotProps.data.withholding.rate }}%</span>
                    <span v-else>-</span>
                </template>
            </Column>
            ...
            <Column header="" width="100" style="width: 20%; min-width: 12rem" bodyStyle="text-align:center">
                <template #body="slotProps">
                    <div>
                        <router-link :to="`/bills/${slotProps.data.id}/items`">
        ...
        <Dialog :modal="true" header="Create Bill" v-model:visible="showModal" class="m-3 md:w-5 w-full md:max-w-screen">
            <Create @toggleModal="toggleModal" :bill="editedItem" :is-edit="isEdit" />
        </Dialog>
    </div>
</template>
