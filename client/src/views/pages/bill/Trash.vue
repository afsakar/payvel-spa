...
async function getPaginateData(page = 1) {
    await axios.get(`/api/v1/bills/${selectedCompany.value.id}/trash?page=${page}&sort=${filters.value.sort}&order=${filters.value.orderBy}&search=${filters.value.search}`).then((res) => {
...
function forceDeleteItem(event, id) {
    confirm.require({
        ...
        header: 'Confirmation',
        accept: () => {
            billStore.forceDeleteBill(id).then(() => {
...
function restoreItem(id) {
    billStore.restoreBill(id).then(() => {
        toast.add({ severity: 'success', summary: 'Successful', detail: 'Bill restored successfully!', life: 3000 });
        setTimeout(() => {
            window.location.reload();
        }, 2000);
    });
