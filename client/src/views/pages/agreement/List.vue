...
const deletedAgreementList = ref([]);
const editedAgreement = ref({});
...
function changeList() {
    ...
    if (!isDeletedList.value) {
        ...
        currentList.value = deletedAgreementList.value;
    }
}
function toggleModal() {
    editedAgreement.value = {};
    isEdit.value = false;
    showModal.value = !showModal.value;
}
function newAgreement(agreement) {
    agreementList.value.push(agreement);
}

function toggleEditModal(agreement) {
    loading.value = true;
    agreementStore.getAgreement(agreement).then(() => {
        editedAgreement.value = agreementStore.agreement.data;
        isEdit.value = true;
        showModal.value = !showModal.value;
        loading.value = false;
    });
...
function deleteItem(event, id) {
    confirm.require({
        ...
        accept: () => {
            agreementStore.deleteAgreement(id).then(() => {
...
<template>
    <Toolbar class="col-12 my-3">
        <template #start>
            <p class="text-xl font-bold">Agreements</p>
        </template>
    ...
    <div class="card">
        ...
        >
            ...
            <Column field="corporation.name" header="Corporation" :sortable="true"></Column>
        ...
        <Dialog :modal="true" header="Create Agreement" v-model:visible="showModal" class="m-3 md:w-5 w-full md:max-w-screen">
            <Create @toggleModal="toggleModal" @newAgreement="newAgreement" :agreement="editedAgreement" :is-edit="isEdit" />
        </Dialog>
    </div>
</template>
