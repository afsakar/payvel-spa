...
useHead({
    title: 'Deleted Withholding List'
});
...
async function getPaginateData(page = 1) {
    await axios.get(`/api/v1/withholdings/trash?page=${page}&sort=${filters.value.sort}&order=${filters.value.orderBy}&search=${filters.value.search}`).then((res) => {
...
<template>
    <Toolbar class="col-12 my-3">
        <template #start>
            <p class="text-xl font-bold">Deleted Withholding List</p>
        </template>

        <template #end>
            <RouterLink to="/withholdings">
