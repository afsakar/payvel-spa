...
onMounted(async () => {
    await getList();
    checkMeta.value = deletedAccountList.value.meta.total >= deletedAccountList.value.meta.per_page ? true : false; // check if there is more data to load
});

async function getList() {
    loading.value = true;
    await getDeletedAccountList();
    loading.value = false;
}

async function getDeletedAccountList(page = 1) {
    await axios.get(`/api/v1/accounts/trash?page=${page}&sort=${filters.value.sort}&order=${filters.value.orderBy}&search=${filters.value.search}`).then((res) => {
        deletedAccountList.value = res.data;
    });
}
...
<template>
    <Toolbar class="col-12 my-3">
        <template #start>
            <p class="text-xl font-bold">Deleted Accounts</p>
        </template>
    ...
    <div class="card">
        <DataTable :loading="loading" ref="dt" dataKey="id" :value="deletedAccountList.data" stripedRows removableSort responsiveLayout="scroll" class="p-datatable-sm">
            <template #header>
            ...
            <template #footer>
                <div v-if="checkMeta" class="border border-1 mt-4 rounded border-[#304562] flex items-center justify-center">
                    <Bootstrap5Pagination :data="deletedAccountList" @pagination-change-page="getDeletedAccountList" />
                </div>
            </template>
        </DataTable>
    </div>
