...
async function getPaginateData(page = 1) {
    await axios.get(`/api/v1/currencies?page=${page}&sort=${filters.value.sort}&order=${filters.value.orderBy}&search=${filters.value.search}`).then((res) => {
...
function toggleEditModal(currency) {
    ...
    currencyStore.getCurrency(currency).then(() => {
        editedItem.value = currencyStore.currency.data;
        isEdit.value = true;
        showModal.value = !showModal.value;
        loading.value = false;
    });
...
<template>
    <Toolbar class="col-12 my-3">
        <template #start>
            <p class="text-xl font-bold">Currency List</p>
        </template>
    ...
    <div class="card">
            ...
            <template #header>
                <div class="table-header">
                    <div class="flex gap-2 justify-content-end">
                        ...
                        <RouterLink to="/currencies/trash">
        ...
        <Dialog :modal="true" header="Create Currency" v-model:visible="showModal" class="m-3 md:w-5 w-full md:max-w-screen">
            <Create @toggleModal="toggleModal" :currency="editedItem" :is-edit="isEdit" />
        </Dialog>
    </div>
</template>
