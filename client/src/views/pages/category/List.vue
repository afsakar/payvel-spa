...
async function getPaginateData(page = 1) {
    await axios.get(`/api/v1/categories?page=${page}&sort=${filters.value.sort}&order=${filters.value.orderBy}&search=${filters.value.search}`).then((res) => {
...
function toggleEditModal(category) {
    ...
    categoryStore.getCategory(category).then(() => {
        editedItem.value = categoryStore.category.data;
        isEdit.value = true;
        showModal.value = !showModal.value;
        loading.value = false;
    });
...
<template>
    ...
    <div class="card">
            ...
            <template #header>
                <div class="table-header">
                    <div class="flex gap-2 justify-content-end">
                        ...
                        <RouterLink to="/categories/trash">
        ...
        <Dialog :modal="true" header="Create Category" v-model:visible="showModal" class="m-3 md:w-5 w-full md:max-w-screen">
            <Create @toggleModal="toggleModal" :category="editedItem" :is-edit="isEdit" />
        </Dialog>
    </div>
</template>
