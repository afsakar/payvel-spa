...
useHead({
    title: 'Deleted Account Types List'
});
...
async function getDeletedList(page = 1) {
    await axios.get(`/api/v1/account-types/trash?page=${page}&sort=${filters.value.sort}&order=${filters.value.orderBy}&search=${filters.value.search}`).then((res) => {
...
<template>
    <Toolbar class="col-12 my-3">
        <template #start>
            <p class="text-xl font-bold">Deleted Account Types</p>
        </template>

        <template #end>
            <RouterLink to="/account-types">
